...
cmd_restart() {
    local vm_name="${1:-}"
    if [ -z "$vm_name" ]; then
        log_err "Missing guest virtual machine identifier. Usage: kvm-blacksmith restart <vm-name>"
        exit 1
    fi
    ...
    # Query VM state first and either reboot if running or start if stopped
    local remote_cmd
    remote_cmd="
        export PATH=\"\$PATH:/usr/sbin:/sbin:/usr/local/bin:/usr/bin:/bin\"
        export LIBVIRT_DEFAULT_URI=\"qemu:///system\"
    ...
    if [ $rc -ne 0 ]; then
        log_err "Failed to restart VM '$vm_name' on remote host $target_anvil."
        exit $rc
    fi

    if [[ "$remote_res" == *"REBOOTING"* ]]; then
        log_info "\033[1;32mGuest VM '$vm_name' successfully rebooted on $target_anvil.\033[0m"
    else
        log_info "\033[1;32mGuest VM '$vm_name' was not running. Successfully booted on $target_anvil.\033[0m"
    fi
}
